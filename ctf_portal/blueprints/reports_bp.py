...
def report_create():
    ...
    if violations:
        flash(
            f"금지된 표현이 포함되어 있습니다. 차단 목록: {', '.join(BLACKLIST)}",
            "danger",
        )
        return redirect(url_for("reports.report_list"))
...
def report_preview(report_id):
    ...
    #
    # 보강: flag_path는 더 이상 템플릿에 노출하지 않음
    # 의도된 풀이: SSTI (attr() + \x5f) 필터 우회를 통한 RCE
    try:
        template_str = (
        ...
        rendered = render_template_string(
            template_str,
            report=report,
            # flag_path 제거 - SSTI 없이는 플래그 파일 경로 획득 불가
            review_flag=flag,  # review_flag는 SSTI로만 접근 가능 (블랙리스트에 있음)
        )
        return rendered
